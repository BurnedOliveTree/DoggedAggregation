...
from server.socketLib import RawSocket
...
class Socket:
    ...
    def read(self):
        ...
        return data, self.client_address
    ...
    def send(self, binary_stream: io.BytesIO, address: str = None) -> None:
        ...
        data = self.split_send_data(binary_stream.read(), self.socket.buffer_size)
    ...
    def split_send_data(self, raw_data: bytes, buffer_size: int) -> str:
        ...
        data.append(self.__create_datagram(raw_data, datagram_amount, datagram_amount - 1,
                                           ((datagram_amount - 1) * max_size, None)))
        return data
